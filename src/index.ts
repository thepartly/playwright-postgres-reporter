...
class PostgresReporter implements Reporter {
	...
	private async db_migrations() {
		try {
			...
			for (const file of sqlFiles) {
				...
				//console.debug(`Executed ${file} successfully.`);
			}
		} catch (error) {
			throw new PostgresReporterError('Failed to migrate database', error);
		}
	}
	...
	private async bootstrap(): Promise<string> {
		...
		let session_insert_result: { id: string }[] = await this._dbClient('test_sessions')
			.insert({
		...
		if (first_row) {
			...
			await fs.writeFile('.postgres-reporter-session-id.txt', sid, { flag: 'w' });
			return sid;
		} else {
	...
	onTestEnd(test: TestCase, result: TestResult) {
		...
		this._sessionID = this._sessionID.then(sessionId => this.report(sessionId, test, result));
	}

	private async report(sessionId: string, test: TestCase, result: TestResult): Promise<string> {
		...
		const testReport = {
			test_name: test_name,
			otel_trace_id: test.annotations.find(a => a.type === 'otel_trace_id')?.description || '',
		...
		if (result.status !== 'passed' && test.titlePath().find(i => i.includes('@experimental'))) {
